...
async function handleSubmit(payload) {
  loading.value = true
  try {
    const { access_token, role } = await register(payload)
    ...
    toastStore.success(`Compte créé — bienvenue, ${payload.username}.`)
...
<template>
  <div class="relative grid min-h-[calc(100vh-4rem)] overflow-hidden lg:grid-cols-2">
    ...
    <div class="relative hidden flex-col justify-between overflow-hidden bg-deep-blue px-12 py-16 text-cream lg:flex">
      ...
      <div class="relative">
        <p class="font-mono text-xs uppercase tracking-[0.4em] text-sage">Commencer</p>
        <h1 class="mt-4 max-w-sm font-display text-5xl font-light leading-tight">
          Dites-nous ce que vous aimez. On vous montre où aller à Bafoussam.
        </h1>
      </div>
      <p class="relative max-w-sm text-sm leading-relaxed text-cream/70">
        Vos préférences guident chaque recommandation : chefferies, marchés, nature… choisissez ce qui vous ressemble.
      </p>
    </div>
    ...
    <div class="flex items-center justify-center px-6 py-16 bg-gradient-to-br from-cream via-lavender-light to-cream">
      <div class="w-full max-w-sm rounded-[28px] border border-white/70 bg-white/70 p-8 shadow-[0_20px_60px_rgba(15,76,92,0.12)] backdrop-blur-sm">
        <h2 class="font-display text-3xl font-light text-deep-blue">Créer votre compte</h2>
        <p class="mt-2 text-sm text-text-secondary">Une minute suffit, puis place à la découverte.</p>

...
function goToCurrentLocation() {
  if (!map.value || !navigator.geolocation) {
    locationStatus.value = 'Geolocation is not available in this browser.'
    return
  }
  ...
  navigator.geolocation.getCurrentPosition(
    (position) => {
      ...
      locationStatus.value = 'Current location updated.'
      map.value.setView(nextLocation, 8)
      if (markersLayer.value) {
        ...
        marker.addTo(markersLayer.value).bindPopup('Your current location')
      }
    },
    (error) => {
      if (error.code === 1) {
        locationStatus.value = 'Location access was denied. Please allow location permissions in your browser.'
      } else {
        locationStatus.value = 'Unable to retrieve your current location right now.'
      }
    },
    {

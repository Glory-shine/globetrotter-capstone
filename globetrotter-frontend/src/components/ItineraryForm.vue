...
<template>
  ...
  >
    <div class="p-6">
      <p class="font-mono text-xs uppercase tracking-[0.3em] text-gold">New itinerary</p>
      <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div class="sm:col-span-2">
          <label for="it-title" class="mb-1.5 block text-sm font-medium text-paper/80">Trip title</label>
          <input
            id="it-title"
            v-model="form.title"
            required
            type="text"
            placeholder="Anniversary getaway"
        ...
        <div class="sm:col-span-2">
          <label for="it-destination" class="mb-1.5 block text-sm font-medium text-paper/80">Destination</label>
          <select
            id="it-destination"
          ...
          >
            <option value="" disabled class="text-ink">Choose a destination…</option>
        ...
        <div>
          <label for="it-start" class="mb-1.5 block text-sm font-medium text-paper/80">Departs</label>
          <input
            id="it-start"
        ...
        <div>
          <label for="it-end" class="mb-1.5 block text-sm font-medium text-paper/80">Returns</label>
          <input
            id="it-end"
      ...
      <p v-else-if="tripLength" class="mt-2 font-mono text-xs text-paper/50">{{ tripLength }} day trip</p>
    </div>
    ...
    <div class="space-y-3 p-6">
      <div class="flex items-center justify-between">
        <span class="text-sm font-medium text-paper/80">Day-by-day plan (optional)</span>
        <button
          type="button"
      ...
      >
        <span class="font-mono text-xs text-gold">Day</span>
        <input
      ...
      >
        {{ submitting ? 'Booking it in…' : 'Create itinerary' }}
      </button>
    </div>
  </form>
</template>
